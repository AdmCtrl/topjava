...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExceed> getFilteredWithExceededTwoOption(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        return mealList.stream()
                .collect(Collectors.groupingBy(userMealRow -> userMealRow.getDateTime().toLocalDate())).values()
                .stream().flatMap(userMealRowGroup -> userMealRowGroup.stream().filter(meal ->
                        TimeUtil.isBetween(meal.getDateTime().toLocalTime(), startTime, endTime))
                        .map(meal -> new UserMealWithExceed(meal.getDateTime(), meal.getDescription(), meal.getCalories(), userMealRowGroup.stream().mapToInt(UserMeal::getCalories).sum() > caloriesPerDay)))
                .collect(Collectors.toList());
    }
}